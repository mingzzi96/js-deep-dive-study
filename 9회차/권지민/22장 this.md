...
function square(number) {
  ...
  // strict mode에서는 undefined가 반환된다. 일반 함수 내에서 this를 사용할 필요가 없기 때문이다.
  console.log(this); // window
...
> ❗️렉시컬 스코프와 this 바인딩은 결정 시기가 다르다. 렉시컬 스코프는 함수 정의가 평가되어 함수 객체가 생성되는 시점에 결정된다. `this`는 함수가 호출 시점에 결정된다.
...
그래서 strict mode의 일반 함수 내부에서 this는 undefined가 바인딩 된다.
...
obj.foo();
```
하지만 콜백 함수의 this가 전역 객체를 바인딩하는 것은 문제가 있다.

아래 3가지 방법을 이용하여 문제를 해결 할 수 있다.
...
메서드 내부의 this는 메서드 이름 앞의 마침표(.) 앞에 기술한 객체가 바인딩 된다. (메서드를 호출한 객체에 바인딩)
...
bind메서드는 apply와 call메서드와 달리 함수를 호출하지는 않고 this로 사용할 객체만 전달한다.
